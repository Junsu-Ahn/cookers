$(document).ready(function () {
          ...
          <div class="top_bar_step">
            ...
            <div class="step_img_bar">
              <div class="PhotoBox" id="divStepPhotoBox_${stepCounter}" is_over="0">
                <div class="input_1" value="${stepCounter}">
                ...
                <img id="upload_main_image" alt="">
                <input type="file" name="file" accept="/image/png" onchange="readURL${stepCounter}(this);">
              </div>
            </div>
          </div>
            <div class="top_bar_tip">
              <div class="tip_name_bar"><i class="fa-solid fa-star"></i> 꿀 팁</div>
            <div class="tip_text_bar"><textarea name="step_text[]" id="step_text_${stepCounter}" placeholder="ex) 고기요리는 소금보다 설탕을 먼저 넣어야 단맛이 겉돌지 않고 육질이 부드러워져요"></textarea></div>
          </div>
  ...
  function updateStepNumbers() {
    ...
    $(".make_step").each(function () {
      ...
      $(this).find(".step_img_bar input[type='file']").attr("onchange", `readURL${currentStep}(this)`);
  ...
  $(".input_1").each(function () {
    ...
    if (value && window[`readURL${value}`]) {
      $(this)
        .parent()
        .find('input[type="file"]')
        .on("change", function () {
          window[`readURL${value}`](this);
        });
    }
  });
});
